...
const balloon_spawn_horizontal_range = [30, 210]
const balloon_spawn_height = 200
const balloon_spawn_timer_seconds = 2
...
func _process(delta):
	if (spawn_timer > balloon_spawn_timer_seconds):
		spawn_timer -= balloon_spawn_timer_seconds
		spawn_balloon()
	spawn_timer += delta
	

func spawn_balloon():
	var new_balloon = meteor_fab.instantiate()
	new_balloon.global_position = Vector2(randf_range(balloon_spawn_horizontal_range[0],balloon_spawn_horizontal_range[1]), balloon_spawn_height)
	new_balloon.speed = meteor_speed
	new_balloon.linear_velocity = Vector2(0,-1)
	meteor_node.add_child(new_balloon)
	
	new_balloon.on_hit.connect(_on_lose)
